...
// Add All Examinee 
$(document).on("submit","#addAllExaminee" , function(){
  $.post("query/addstdsheet.php", $(this).serialize() , function(data){
    if(data.res == "exist")
    {
      Swal.fire(
        'Already Exist',
...
//add faculty
$(document).on("submit","#addFacultyFrm" , function(){
  $.post("query/addFaculty.php", $(this).serialize() , function(data){
    if(data.res == "noGender")
    {
      Swal.fire(
          'No Gender',
...
function refreshDiv()
{
  $('#tableLisexampleModalLabelt').load(document.URL +  ' #tableList');
